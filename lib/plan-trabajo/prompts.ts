...
function evidenceRules() {
  return [
    ...
    "2) Cada decision del plan debe nacer de al menos una fuente: metadata_json, BEC, brief, documentos ya cargados de empresa, documentos ya cargados de agencia, apoyo escrito por el usuario, links pegados por el usuario o investigacion web.",
    ...
    "5) Si hay conflicto entre fuentes, prioriza en este orden: metadata_json operativo > apoyo de esta generacion > brief seleccionado > BEC > documentos de empresa > investigacion web > documentos de agencia > prompts generales.",
    ...
    "7) En _estrategia_oculta redacta primero un analisis de 3 parrafos antes de llenar el resto del plan. Cruza 1) una friccion real del cliente extraida de metadata_json, documentos, BEC, brief o apoyo manual, 2) una ventaja del producto segun el BEC y 3) como la investigacion web dicta el tono del mes. Si este campo es generico, el resto del plan fallara.",
    "8) En notas_adicionales agrega una linea 'Fuentes usadas:' con las fuentes realmente usadas: metadata_json, BEC, brief, documentos ya cargados, apoyo manual, links o investigacion web.",
  ];
}
...
function antiGenericRules() {
  return [
    ...
    "2) Una idea valida debe contener al menos uno de estos elementos: producto/servicio concreto, audiencia concreta, objecion concreta, situacion de uso, prueba, diferenciador, tension competitiva, temporalidad, restriccion operativa o insight documental.",
    ...
    "6) Productos_servicios_destacar debe salir de BEC, brief, metadata_json, documentos, apoyo manual o web. Si no hay productos/servicios claros, dilo y pide validacion.",
  ];
}
...
export function buildPlanTrabajoPrompt(params: {
  ...
  return [
    ...
    params.briefContext,
    "",
    "CONTEXTO DE APOYO PARA ESTA GENERACION (texto escrito por el usuario):",
    params.supportDocContext || "Sin contexto de apoyo",
    "",
    ...
    "",
    "CONOCIMIENTO DE AGENCIA YA CARGADO:",
    params.agenciaDocsContext || "Sin documentos globales de agencia cargados",
    "",
    "PROMPTS ACTIVOS:",
    ...
    "",
    "REGLAS CLAVE:",
    "0) El plan debe salir de una sintesis real entre DATA INTERNA y WEB: metadata_json, BEC, brief seleccionado, documentos ya cargados de empresa/agencia, apoyo manual de esta generacion, links pegados e investigacion web. Ninguna seccion estrategica debe generarse solo con plantillas generales.",
    "1) Ajusta el contenido al contexto de Peru y al rubro real de la empresa.",
    "2) Manten coherencia total con BEC + BRIEF + metadata_json + documentos ya cargados + apoyo manual + investigacion web.",
    "3) Usa los documentos ya cargados sin exigir que el usuario suba archivos nuevos. Si existe apoyo escrito para esta generacion, usalo como insumo prioritario para completar y afinar el plan.",
    ...
    "15) En notas_adicionales incluye una linea llamada 'Fuentes usadas' que resuma que se cruzo metadata_json, BEC, brief, documentos ya cargados, apoyo manual, links e investigacion web, sin pegar URLs largas.",
    "16) No completes arrays por completitud. Completa solo con decisiones defendibles desde las fuentes.",
    "17) Usa nombres reales, temas reales, lineamientos reales y restricciones reales cuando esten en metadata_json, BEC, brief, documentos ya cargados, apoyo manual o investigacion web. No los reemplaces por abstracciones.",
    ...
    "19) ANCLAJE OBLIGATORIO: esta prohibido usar conceptos abstractos como 'mejorar engagement' o 'post institucional'. Cada pilar, mensaje y producto a destacar debe incluir un nombre propio, un dato tecnico, una objecion real o un beneficio especifico extraido de metadata_json, BEC, brief, documentos ya cargados, apoyo manual o web.",
    "",
    "REGLAS POR SECCION:",
    ...
    "2) resumen_actualizaciones: redacta una sintesis ejecutiva con diagnostico y criterio. Debe mencionar 2 a 4 evidencias concretas de BEC, brief, metadata_json, documentos ya cargados, apoyo manual o web. observaciones debe contener hallazgos, alertas, oportunidades o decisiones claras.",
    ...
    "6) productos_servicios_destacar y mensajes_destacados: prioriza lo mas vendible, diferenciador o relevante para el periodo segun BEC, brief, metadata_json, documentos ya cargados, apoyo manual o web. Jerarquiza con criterio comercial.",
    ...
    "10) notas_adicionales: resume decisiones estrategicas, tensiones, riesgos, oportunidades o focos de ejecucion. Incluye 'Fuentes usadas:' y 'Pendientes de validacion:' si aplica.",
