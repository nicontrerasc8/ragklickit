...
function userGenerationContextBlock(context?: {
  ...
  return [
    ...
    "Los LINKS INDICADOS y sus EXTRACTOS LEIDOS son fuente primaria del post. El contenido final debe tomar de ahi informacion, lenguaje, beneficios, restricciones, pruebas y angulos antes que de cualquier busqueda general.",
    "Si hay conflicto entre los links indicados y la investigacion general de empresa, prioriza los links indicados.",
    "",
...
export function buildCalendarioStudioPrompt(params: {
  ...
  const previousBundleBlock = existingBundle
    ? [
        "CONTENIDO ANTERIOR (solo como referencia de tono, no reutilizar):",
        "No copies, no parafrasees y no uses este contenido como respuesta correcta. Usalo solo para entender continuidad de estilo si aporta.",
        compactJson({
  ...
  return [
    ...
    "",
    "ITEM A PRODUCIR:",
    "Este item es la fuente primaria de verdad. Todo el contenido generado debe derivar directamente de su tema, subtema, mensaje_clave y buyer_persona.",
    compactJson(itemShapeFromAI(item)),
    "",
    ...previousBundleBlock,
    "INVESTIGACION WEB OBLIGATORIA - USAR PARA: lenguaje real del mercado, objeciones frecuentes, referencias culturales, SEO, competencia, benchmarks y angulos de diferenciacion.",
    params.webResearchContext || "Sin investigacion web disponible",
    "Si el bloque incluye 'FUENTE PRIMARIA DEL POST - LINKS PEGADOS POR EL USUARIO', esos links mandan sobre la investigacion general de empresa.",
    "Si hay investigacion especifica de content studio, usala como fuente principal de angulos, objeciones, lenguaje, referencias y riesgos para esta pieza.",
    "Si no hay investigacion, deriva el lenguaje del tema y buyer_persona del item.",
    "",
    ...
    "5) Usar investigacion web para aterrizar lenguaje, competencia, SEO, objeciones, escenas de uso, benchmarks y referencias culturales; no inventar precios, promociones, fechas ni claims duros.",
    ...
    "7) Si hay extractos leidos de links, el copy debe reflejar datos, lenguaje o angulos concretos de esos extractos cuando sean relevantes para el item.",
    "8) Si un link no pudo leerse, no inventes su contenido: usa solo la URL y la informacion pegada por el usuario.",
    "9) Todo angulo creativo debe estar conectado a algun dato del item, investigacion web, links o instrucciones del usuario; evita ideas decorativas sin sustento.",
    "10) Cuando haya links indicados para la pieza, headline, caption, guion, carrusel, blog/email y CTA deben estar anclados a esos links; no generes una pieza generica de marca.",
    "",
    ...premiumCreativeRules(),
    "",
    "CRITERIOS DE CALIDAD:",
    "1) Antes de escribir el headline, identifica: cual es la tension, deseo, objecion o insight central del tema para este buyer_persona. Usalo como eje. Si el headline no refleja ese eje, descartalo.",
    "1.1) headline, caption y short_copy deben ser especificos para el tema y evitar lugares comunes.",
    "1.2) headline debe poder competir por atencion en el feed sin sonar clickbait vacio.",
    "2) hashtags debe ser un array de hashtags limpios y relevantes.",
    "2.1) Si usas un beneficio, promesa, restriccion, producto, servicio o dato operativo, debe venir del item, de informacion pegada por el usuario, de extractos de links o de investigacion web verificable.",
    ...
    compactJson({
      ...
      short_copy: "<copy corto aqui>",
      blog_title: "<titulo de blog aqui o vacio si no aplica>",
      blog_body_markdown: "<cuerpo de blog aqui o vacio si no aplica>",
      email_subject: "<subject aqui o vacio si no aplica>",
      email_preheader: "<preheader aqui o vacio si no aplica>",
      email_body_markdown: "<cuerpo de email aqui o vacio si no aplica>",
      video_hook: "<hook de video aqui o vacio si no aplica>",
      video_script: "<guion de video aqui o vacio si no aplica>",
      carousel_slides: ["<slide 1>", "<slide 2>"],
      cta: "<cta aqui>",
      hashtags: [],
      visual_direction: "<direccion visual aqui>",
      image_prompt_base: "<prompt visual base aqui>",
    }),
    isLongform
...
export function buildCalendarioDraftPrompt(params: {
  ...
  return [
    ...
    "",
    "REGLAS DURAS SOBRE IDEAS SELECCIONADAS:",
    "1) plan_trabajo.contenido_sugerido contiene solo las ideas seleccionadas en el checklist del plan.",
    "2) calendario.items debe tener exactamente la misma cantidad de items que ESTRUCTURA EXACTA REQUERIDA.calendario.items.",
    "3) No agregues, elimines, dividas, fusiones ni reemplaces ideas.",
    "4) Cada idea seleccionada debe convertirse en exactamente un item de calendario.",
    "5) Conserva el canal de cada idea seleccionada. No muevas ideas entre canales.",
    "6) Conserva el orden de ESTRUCTURA EXACTA REQUERIDA.calendario.items.",
    "7) Puedes afinar campos operativos como CTA, mensaje_clave, buyer_persona o hashtags, pero titulo_base, tema y subtema deben seguir anclados a la idea seleccionada.",
    "",
