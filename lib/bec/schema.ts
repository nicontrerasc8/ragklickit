...
function sanitizeBecField(_key: BECFieldKey, value: string) {
  return value.trim();
}
...
export function mapAnswerToBec(answer: string, base?: BECState): BECState {
  ...
  (Object.keys(nextFields) as BECFieldKey[]).forEach((key) => {
    ...
    if (value) {
      nextFields[key] = sanitizeBecField(key, value);
    }
  });
  ...
  if (obj.fields && typeof obj.fields === "object") {
    for (const key of Object.keys(next.fields) as BECFieldKey[]) {
      ...
      if (typeof value === "string") {
        next.fields[key] = sanitizeBecField(key, value);
      }
    }
  }
...
export function buildPromptBEC(
  ...
  const objectiveSmartRule =
    '- En todo el bloque "3. Objetivos Estrategicos y KPIs" ("Objetivo SMART 1", "Objetivo SMART 2", "KPI Primario", "KPI Secundario" y "Métrica de Éxito / ROI Esperado"), no escribas "Pendiente de validar" ni "SUPUESTO:" como salida final. Lee metadata_json, el BEC actual y el contexto documental para proponer objetivos, indicadores y metrica de exito coherentes.\n- En "Objetivo SMART 1" y "Objetivo SMART 2", no incluyas porcentajes, umbrales ni metas numericas inventadas. Formula cada objetivo con accion, foco de negocio, resultado esperado y criterio de medicion ligado a los KPIs.\n- En "KPI Primario" y "KPI Secundario", elige indicadores defendibles a partir del objetivo, industria, alcance, entregables y problema de negocio; no inventes valores meta.\n- En "Métrica de Éxito / ROI Esperado", describe como se evaluara el resultado segun el KPI primario, sin convertirlo en porcentaje por defecto salvo que el contexto lo exija explicitamente.';
